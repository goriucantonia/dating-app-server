...
- **Built 2026-09-01 (Step 15), inline revisions:** (a) `analyses.candidate_count` (migration 0009) remembers how many people an analysis HAD, because once a candidate's row cascades away nothing else does; `GET /analyses/{id}` carries `removed_candidates` and the UI renders the tombstone from it. (b) `DELETE /me` returns `{deleted: {table: n}, rows_removed}` — the counts logged before the cascade, as the receipt; the count list is checked by a unit test against the ORM's cascade graph, so a new table that hangs off `users` cannot go uncounted. (c) Step 2's AI half (extraction, compilation, embedding) runs in a **background task after boot** — accounts and answers are seeded inline; three model calls per profile must not hold the health endpoint hostage. Scripts and probes await it inline (`run_full_pass(inline_demo_pipeline=True)`). A demo user whose extraction lock is held by another run is `deferred`, never compiled from nothing (D-015). (d) Step 4 "queued for re-embedding" is spelled: the mismatched row's `traits_hash` is blanked, so `refresh_embeddings` regenerates it with the pinned model before its next comparison.
- **Persona identity repair (D-017, added 2026-09-02).** A compiled persona prompt states its owner's name, age, gender, interested-in and city. When the owner edits one, the prompt is wrong until it is re-issued — and the traits-based staleness rule cannot see it. Reconciliation now sweeps every user's current snapshot, compares the frozen facts block to the live profile, and re-issues a new immutable version where they differ. No model calls; logs `reconcile_identities` with counts, or `reconcile_identities_noop` on a healthy pass.
...
- **Startup reconciliation pass** (one place, ordered):
  ...
  3. Persona identity repair (D-017): re-issue any snapshot whose stated facts have drifted from its owner's profile.
  4. Re-launch analyses stuck in `matching`/`simulating` (the resume pass from `date_simulation.md`).
  5. Embedding-model consistency check: every `profile_embeddings.embedding_model` must equal the pinned config value; mismatches are logged loudly and queued for re-embedding, never silently compared.
